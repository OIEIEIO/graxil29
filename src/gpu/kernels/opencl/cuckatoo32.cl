// src/gpu/kernels/opencl/cuckatoo32.cl - Cuckatoo32 OpenCL Mining Kernels
// Tree location: ./src/gpu/kernels/opencl/cuckatoo32.cl

//! Cuckatoo32 OpenCL implementation adapted from Cuckaroo29
//! 
//! Original: Cuckatoo Cycle, a memory-hard proof-of-work by John Tromp and team Grin
//! Copyright (c) 2018 Jiri Photon Vadura and John Tromp
//! This GGM miner file is covered by the FAIR MINING license
//! 
//! Adapted for Graxil29 miner with Cuckatoo32 specifications
//! 
//! # Algorithm Overview
...
//! 4. FluffyRoundN*: 130+ trimming rounds to reduce graph
//! 5. FluffyTail: Collect remaining edges after trimming
//! 6. FluffyRecovery: Recover nonces from solution cycles
//! 
//! # Memory Layout (Cuckatoo32)
//! - Buffer A1: Primary edge storage (DUCK_SIZE_A * 1024 * 8192 * 2)
//! - Buffer A2: Secondary edge storage (DUCK_SIZE_A * 1024 * 512 * 2)  
//! - Buffer B:  Bucket storage (DUCK_SIZE_B * 1024 * 8192 * 2)
...
// Cuckatoo32 algorithm constants - larger than C29
#define DUCK_SIZE_A 129L          // AMD optimized: 126 + 3
#define DUCK_SIZE_B 85L           // Slightly larger bucket size for C32
...
// Cuckatoo32 specific: 32-bit edge space (much larger than C29)
#define EDGEBITS 32
#define NEDGES ((node_t)1 << EDGEBITS)    // 2^32 = 4,294,967,296 edges
#define EDGEMASK (NEDGES - 1)             // Mask for 32-bit values
...
//! Adapted for Cuckatoo32's larger edge space.
//! 
...
//! Adapted for Cuckatoo32's memory requirements.
//! 
...
//! Adapted for Cuckatoo32's larger node space.
//! 
                            ...
                            __global int * destinationIndexes, const int bktInSize, const int bktOutSize)
{
	...
	// First pass: count edge degrees - adjusted for 32-bit space
	for (int i = 0; i < loops; i++)
	{
		...
		if (lindex < edgesInBucket)
		{
			...
			// Use higher bits for bucketing in 32-bit space
			Increase2bCounter(ecounters, (edge.x & EDGEMASK) >> 15);
		}
	}
	...
	for (int i = 0; i < loops; i++)
	{
		...
		if (lindex < edgesInBucket)
		{
			...
			if (Read2bCounter(ecounters, (edge.x & EDGEMASK) >> 15))
			{
...
//! 
//! Implements the main trimming algorithm. Runs for ~130 iterations to
//! progressively remove low-degree nodes until only cycles remain.
//! Adapted for Cuckatoo32's requirements.
//! 
                            ...
                            const __global int * sourceIndexes, __global int * destinationIndexes)
{
	...
	// Count degrees - adjusted for 32-bit space
	for (int i = 0; i < loops; i++)
	{
		const int lindex = (i * CTHREADS) + lid;
